...
class DeepBLSTMSegment():
	...
	def build_graph(self):
		with tf.name_scope('embedding_layer'):
			...
			self.encoder_embedding = tf.get_variable(name = 'embedding', shape = [len(self.dictionary), self.embedding_size], initializer = initializer, dtype = tf.float64)
			...
			# input layer
			input_layer = Dense(self.n_hidden, dtype = tf.float64, name = 'input_projection')
			self.inputs_embedded = input_layer(self.inputs_embedded)
		
		### LSTM layer
		with tf.name_scope("biLSTM_layer"):
			self.output = self.inputs_embedded
			self.lstm_cells_fw = [tf.contrib.rnn.DropoutWrapper(BNLSTMCell(num_units = self.n_hidden, is_training_tensor = self.is_training, max_bn_steps = self.max_step, dtype = tf.float64), output_keep_prob = self.dropout) for i in range(self.num_layers)]
			self.lstm_cells_bw = [tf.contrib.rnn.DropoutWrapper(BNLSTMCell(num_units = self.n_hidden, is_training_tensor = self.is_training, max_bn_steps = self.max_step, dtype = tf.float64), output_keep_prob = self.dropout) for i in range(self.num_layers)]
			# self.lstm_cells_fw = [BNLSTMCell(num_units = self.n_hidden, is_training_tensor = self.is_training, max_bn_steps = self.max_step, dtype = tf.float64) for i in range(self.num_layers)]
			# self.lstm_cells_bw = [BNLSTMCell(num_units = self.n_hidden, is_training_tensor = self.is_training, max_bn_steps = self.max_step, dtype = tf.float64) for i in range(self.num_layers)]
		...
		with tf.name_scope("softmax_layer"):
			...
			self.w = tf.get_variable(shape = [2*self.n_hidden, self.num_classes], name = 'w', initializer = tf.contrib.layers.xavier_initializer(), dtype = tf.float64)
			self.b = tf.get_variable(shape = [1, self.num_classes], name = 'b', initializer = tf.contrib.layers.xavier_initializer(), dtype = tf.float64)
	...
	def evaluate_f1(self, x_test, y_test, seqlen_test, batch_size):
		...
		while (start < len(seqlen_test)):
			...
			batch_seqlen = seqlen_test[start:start + batch_size]
			start += batch_size
			_pred = self.sess.run(self.pred, feed_dict = {self.x: batch_data, self.sequence_length: batch_seqlen, self.dropout: 1.0, self.is_training: False})
			for i in range(len(batch_seqlen)):
	...
	def train_new_model(self, x_train, y_train, seqlen_train, x_valid, y_valid, seqlen_valid, lr, dropout, batch_size, num_epochs, save_path):
		...
		for epoch in range(num_epochs):
			...
			while (len(x) != 0):
				...
				_loss, _ = self.sess.run([self.loss, self.optimizer], feed_dict = {self.x: x_batch, self.y: y_batch, self.sequence_length: seqlen_batch, self.lr: lr, self.dropout: dropout, self.is_training: True})
				sum_loss += _loss
...
segmenter.load_model("models/embed_blstm/xavier-batch-norm_2_layer_cell_200_lr_0.01_dropout_0.5/model42.ckpt")
print(segmenter.evaluate_f1(test_data, test_label, test_seqlen, batch_size))


"""
batch-norm_2_layer_cell_100     lr = 0.1
0.9586284707889595, 0.9624955101815268, 0.9605580984944576
batch-norm_2_layer_cell_100_lr_0.01
0.9586694710944408, 0.9646450971182273, 0.9616480011458225
batch-norm_2_layer_cell_200_lr_0.01_dropout_0.8
0.9592113647849876, 0.9648882380570829, 0.9620414268831216
batch-norm_2_layer_cell_200_lr_0.01_dropout_0.5
0.965369825582036, 0.9683198408532037, 0.9668425829625114
xavier-batch-norm_2_layer_cell_200_lr_0.01_dropout_0.5/model42
0.9620131387262583, 0.9670101953416407, 0.9645051946978257
"""
