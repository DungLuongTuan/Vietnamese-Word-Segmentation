...
class DeepCNNBLSTMSegment():
	...
	def predict(self, word_inputs, char_inputs, seqlen):
		_pred = self.sess.run(self.viterbi_sequence, feed_dict = {self.word_inputs: [word_inputs], self.char_inputs: [char_inputs], self.sequence_length: [seqlen], self.dropout: 1.0, self.is_training: False})
	...
	def evaluate(self, word_inputs_test, char_inputs_test, y_test, seqlen_test, batch_size):
		...
		while (start < len(seqlen_test)):
			...
			start += batch_size
			_pred = self.sess.run(self.viterbi_sequence, feed_dict = {self.word_inputs: batch_word_inputs_data, self.char_inputs: batch_char_inputs_data, self.sequence_length: batch_seqlen, self.dropout: 1.0, self.is_training: False})
...
save_path = "models/BIE_cnn_blstm_crf/normal_2_layer_cell_200_ce_64_we_64_lr_0.001_dropout_0.8_cnn_30_30_40"
...
test_word_data, test_char_data, test_label, test_seqlen = load_data("data/VTB-train-dev-test/test-BIE")
...
# print(sentences[0])
# for word_data, char_data, seqlen in zip(test_word_data, test_char_data, test_seqlen):
# 	print(list(segmenter.predict(word_data, char_data, seqlen)))
# 	print(list(test_label[0][:test_seqlen[0]]))
# 	break
