...
class DeepBLSTMSegment():
	...
	def train_new_model(self, x_train, y_train, seqlen_train, x_valid, y_valid, seqlen_valid, lr, dropout, batch_size, num_epochs, save_path):
		...
		for epoch in range(num_epochs):
			...
			acc_train = self.evaluate(x_train, y_train, seqlen_train, batch_size)
			acc_test = self.evaluate(x_valid, y_valid, seqlen_valid, batch_size)
			print('epoch: ', epoch, ' loss: ', sum_loss/(int((len(x_train)-1)/batch_size) + 1), ' acc_train: ', acc_train, ' acc_test: ', acc_test, ' time: ', str(datetime.now() - start_time))
			f.write('epoch: ' + str(epoch) + ' loss: ' + str(sum_loss/(int((len(x_train)-1)/batch_size) + 1)) + " acc_train: " + str(acc_train) + ' acc_test: ' + str(acc_test) + ' time: ' + str(datetime.now() - start_time) + "\n")
...
batch_size = 128
num_epochs = 200
save_path = "models/embed_blstm/xavier-batch-norm_3_layer_cell_200_lr_0.01_dropout_0.5"
...
# segmenter.load_model("models/embed_blstm/batch-norm_2_layer_cell_200_lr_0.01_dropout_0.5/model.ckpt")
