...
""" author: https://r2rt.com/implementing-batch-normalization-in-tensorflow.html"""
...
class BNLSTMCell(RNNCell):
	...
	def __init__(self, num_units, is_training_tensor, max_bn_steps, initial_scale=0.1, activation=tf.tanh, decay=0.95, dtype = tf.float32):
		"""
	...
	def _batch_norm(self, x, name_scope, step, epsilon=1e-5, no_offset=False, set_forget_gate_bias=False):
		...
		with tf.variable_scope(name_scope):
			...
			scale = tf.get_variable('scale', [size], initializer=tf.constant_initializer(self._initial_scale), dtype = self._dtype)
			if no_offset:
				offset = 0
			elif set_forget_gate_bias:
				offset = tf.get_variable('offset', [size], initializer=offset_initializer(), dtype = self._dtype)
			else:
				offset = tf.get_variable('offset', [size], initializer=tf.zeros_initializer, dtype = self._dtype)

			pop_mean_all_steps = tf.get_variable('pop_mean', [self._max_bn_steps, size], initializer=tf.zeros_initializer, trainable=False, dtype = self._dtype)
			pop_var_all_steps = tf.get_variable('pop_var', [self._max_bn_steps, size], initializer=tf.ones_initializer(), trainable=False, dtype = self._dtype)
	...
	def __call__(self, x, state, scope=None):
		with tf.variable_scope(scope or type(self).__name__):
			...
			W_xh = tf.get_variable('W_xh',
				...
				initializer=orthogonal_lstm_initializer(), dtype = self._dtype)
...
def orthogonal_lstm_initializer():
	def orthogonal(shape, dtype=tf.float64, partition_info=None):
		...
		q = q.reshape(shape)
		return tf.constant(q[:shape[0], :shape[1]], tf.float64)
	return orthogonal
...
def offset_initializer():
	def _initializer(shape, dtype=tf.float64, partition_info=None):
		...
		return tf.constant(np.concatenate(res, axis=0), tf.float64)
	return _initializer