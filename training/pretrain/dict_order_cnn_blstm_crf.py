        ...
        for row in f:
            if (row == "\n") and (len(wordvecs) != 0):
                ...
                if word_vector_model == "ft":
                    ...
                    wordvecs.append(word2vec.get(word))
                charvec = []
                for char in word:
...
save_path = "models/pretrain/pmi_dict_order_cnn_blstm_crf/lm_normal_2_layer_cell_200_we_100_ce_100_lr_0.0005_dropout_0.8_cnn_1_30_2_30_3_40"
...
with open('data/pmi/train_dict_order_feats.pkl', 'rb') as f:
    train_dict_order_feats = pickle.load(f)
    train_dict_order_feats = np.append(train_dict_order_feats[:,:,1:2], train_dict_order_feats[:,:,3:4], axis = -1)
...
with open('data/pmi/dev_dict_order_feats.pkl', 'rb') as f:
    dev_dict_order_feats = pickle.load(f)
    dev_dict_order_feats = np.append(dev_dict_order_feats[:,:,1:2], dev_dict_order_feats[:,:,3:4], axis = -1)
